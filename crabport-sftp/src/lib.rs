...
/// trait: they depend on internals of [`SftpBackend`] (access to the
...
impl SftpBackend {
    ...
    /// [`upload_file_gz`]) and decompress it in transit, writing the
    /// original bytes to `local_path`.
    ///
    /// This is the **two-phase** version: download the `.gz` to a local tmp
    /// file first (via the parallel-segmented [`download_file`], so we get
    /// full SFTP throughput), then decompress locally with no network in
    /// the loop. The previous streaming version (`GzipDecoder` over the
    /// live SFTP file) was slow because each `decoder.read()` blocked on a
    /// network round-trip and the gzip format can't be split into parallel
    /// segments.
    ///
    ...
    /// to `remote_tar_gz`.
    ///
    /// This is the directory analogue of [`upload_file_gz`]: the remote
    /// `remote_tar_gz` path ends up holding a tar.gz byte stream. The caller
    /// (typically `SshBackend`'s dir-upload orchestration) is responsible
    /// for extracting it on the remote via `tar xzf`.
    ///
    ...
    /// that tmp file via the existing [`upload_file`]. This costs one local
    /// disk write of the compressed archive — an acceptable trade-off vs.
    /// writing an async `Write` bridge for `tar`.
    ...
    ///
    /// This is the directory analogue of [`download_file_gz`]. The caller
    /// (typically `SshBackend`'s dir-download orchestration) is responsible
    /// for creating the remote `.tar.gz` via `ssh exec` `tar czf` first.
    ///
    /// Implementation: same tmp-file approach as [`upload_dir`] — download
    /// the archive to a local tmp file, then unpack with `tar::Archive`.
...
fn build_tar_gz(local_dir: &str, archive_name: &str, out_path: &std::path::Path) -> Result<()> {
...
fn local_tmp_path(suffix: &str) -> std::path::PathBuf {
